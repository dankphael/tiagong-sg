...
export default function NetworkPage() {
  ...
  const [everyoneDialectFilter, setEveryoneDialectFilter] = useState("All");
  const [everyoneGenderFilter, setEveryoneGenderFilter] = useState("All");
  const [everyoneAgeFilter, setEveryoneAgeFilter] = useState("All");
  const [sinsehSearch, setSinsehSearch] = useState("");
  const [sinSehGenderFilter, setSinSehGenderFilter] = useState("All");
  const [sinSehAgeFilter, setSinSehAgeFilter] = useState("All");
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }} className="fade-up">
      ...
      {networkView === "directory" && (
        <div>
          ...
          {/* Dialect filter pills + search */}
          <div style={{ position: "relative", maxWidth: 480, margin: "0 auto 16px" }}>
            <Search size={16} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", color: "#B8A898" }} />
            <input type="text" value={sinsehSearch} onChange={e => setSinsehSearch(e.target.value)}
              placeholder="Search mentors by name or username..."
              className="input" style={{ width: "100%", padding: "14px 16px 14px 44px", borderRadius: 12, border: "2px solid #E8DDD0", fontSize: 14, fontFamily: "inherit", outline: "none", background: "white", boxSizing: "border-box" }} />
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, justifyContent: "center" }}>
            {["All", "Hokkien", "Cantonese", "Teochew", "Hakka", "Hainanese"].map(f => (
              <button key={f} onClick={() => setSinSehDialectFilter(f)} className={sinSehDialectFilter === f ? "active" : ""}
                ...
                {f === "All" ? "All Dialects" : f}
              </button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, justifyContent: "center" }}>
            {["All", "Male", "Female"].map(f => (
              <button key={f} onClick={() => setSinSehGenderFilter(f)}
                style={{ padding: "7px 14px", borderRadius: 20, background: sinSehGenderFilter === f ? "#C0392B" : "white", color: sinSehGenderFilter === f ? "white" : "#6B5B45", fontSize: 12, border: "1px solid " + (sinSehGenderFilter === f ? "#C0392B" : "#E8DDD0"), fontWeight: sinSehGenderFilter === f ? 600 : 400, cursor: "pointer", fontFamily: "inherit" }}>
                {f === "All" ? "All Genders" : f}
              </button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, justifyContent: "center" }}>
            {["All", "Under 18", "18–24", "25–34", "35–44", "45+"] .map(f => (
              <button key={f} onClick={() => setSinSehAgeFilter(f)}
                style={{ padding: "7px 14px", borderRadius: 20, background: sinSehAgeFilter === f ? "#C0392B" : "white", color: sinSehAgeFilter === f ? "white" : "#6B5B45", fontSize: 12, border: "1px solid " + (sinSehAgeFilter === f ? "#C0392B" : "#E8DDD0"), fontWeight: sinSehAgeFilter === f ? 600 : 400, cursor: "pointer", fontFamily: "inherit" }}>
                {f === "All" ? "All Ages" : f}
              </button>
            ))}
          </div>
      ...
      {networkView === "everyone" && (
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ position: "relative", maxWidth: 480, margin: "0 auto 16px" }}>
              <Search size={16} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", color: "#B8A898" }} />
              <input type="text" value={everyoneSearch} onChange={e => setEveryoneSearch(e.target.value)}
                placeholder="Search by name or username..."
                className="input" style={{ width: "100%", padding: "14px 16px 14px 44px", borderRadius: 12, border: "2px solid #E8DDD0", fontSize: 14, fontFamily: "inherit", outline: "none", background: "white", boxSizing: "border-box" }} />
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center" }}>
              {["All", "Hokkien", "Cantonese", "Teochew", "Hakka", "Hainanese"].map(f => (
                <button key={f} onClick={() => setEveryoneDialectFilter(f)}
                  style={{ padding: "7px 14px", borderRadius: 20, background: everyoneDialectFilter === f ? "#C0392B" : "white", color: everyoneDialectFilter === f ? "white" : "#6B5B45", fontSize: 12, border: "1px solid " + (everyoneDialectFilter === f ? "#C0392B" : "#E8DDD0"), fontWeight: everyoneDialectFilter === f ? 600 : 400, cursor: "pointer", fontFamily: "inherit" }}>
                  {f === "All" ? "All Dialects" : f}
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginTop: 10 }}>
              {["All", "Mentor", "Mentee", "Both"].map(f => (
                <button key={f} onClick={() => setNetworkFilter(f)}
                  style={{ padding: "7px 14px", borderRadius: 20, background: networkFilter === f ? "#C0392B" : "white", color: networkFilter === f ? "white" : "#6B5B45", fontSize: 12, border: "1px solid " + (networkFilter === f ? "#C0392B" : "#E8DDD0"), fontWeight: networkFilter === f ? 600 : 400, cursor: "pointer", fontFamily: "inherit" }}>
                  {f === "All" ? "All Roles" : f}
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginTop: 10 }}>
              {["All", "Male", "Female"].map(f => (
                <button key={f} onClick={() => setEveryoneGenderFilter(f)}
                  style={{ padding: "7px 14px", borderRadius: 20, background: everyoneGenderFilter === f ? "#C0392B" : "white", color: everyoneGenderFilter === f ? "white" : "#6B5B45", fontSize: 12, border: "1px solid " + (everyoneGenderFilter === f ? "#C0392B" : "#E8DDD0"), fontWeight: everyoneGenderFilter === f ? 600 : 400, cursor: "pointer", fontFamily: "inherit" }}>
                  {f === "All" ? "All Genders" : f}
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginTop: 10 }}>
              {["All", "Under 18", "18–24", "25–34", "35–44", "45+"] .map(f => (
                <button key={f} onClick={() => setEveryoneAgeFilter(f)}
                  style={{ padding: "7px 14px", borderRadius: 20, background: everyoneAgeFilter === f ? "#C0392B" : "white", color: everyoneAgeFilter === f ? "white" : "#6B5B45", fontSize: 12, border: "1px solid " + (everyoneAgeFilter === f ? "#C0392B" : "#E8DDD0"), fontWeight: everyoneAgeFilter === f ? 600 : 400, cursor: "pointer", fontFamily: "inherit" }}>
                  {f === "All" ? "All Ages" : f}
                </button>
              ))}
            </div>
                  ...
                  return (
                    <div key={m.id} className="card" style={{ padding: 24 }}>
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                        ...
                        {m.gender && <span style={{ fontSize: 11, background: m.gender === "male" ? "#EEF2FF" : "#FDEDEC", color: m.gender === "male" ? "#3B5998" : "#C0392B", padding: "3px 10px", borderRadius: 12, fontWeight: 600, textTransform: "capitalize" }}>{m.gender}</span>}
                      </div>
                      {connStatus === 'accepted' ? (
                        <button className="btn-hover" onClick={() => openChatWith(m.id)}
